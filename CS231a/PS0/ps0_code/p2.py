...
def part_b(img1, img2):
    ...
    # BEGIN YOUR CODE HERE
    img1 = normalize_img(img1.astype(np.double))
    img2 = normalize_img(img2.astype(np.double))
...
def part_d(img1, img2):
    ...
    newImage1[:, :W//2, :] = img1[:, :W//2, :]
    newImage1[:, W//2:, :] = img2[:, W//2:, :]
    # END YOUR CODE HERE
...
def part_e(img1, img2):    
    ...
    for i in range(img1.shape[0]):
        newImage2[i] = img2[i] if i%2 else img1[i]
    # END YOUR CODE HERE
...
def part_f(img1, img2):     
    ...
    # BEGIN YOUR CODE HERE
    img1_mask = np.tile(np.array([1, 0]), (3, img1.shape[1], img1.shape[0]//2)).transpose(2, 1, 0)
    img2_mask = np.tile(np.array([0, 1]), (3, img2.shape[1], img2.shape[0]//2)).transpose(2, 1, 0)
    newImage3 = img1*img1_mask + img2*img2_mask
    # END YOUR CODE HERE
...
def part_g(img):         
    ...
    assert (grayImage == img[..., 0]*0.299 + img[..., 1]*0.587 + img[..., 2]*0.114).all()
    # END YOUR CODE HERE
...
if __name__ == '__main__':
    ...
    assert (newImage2 == newImage3).all()
